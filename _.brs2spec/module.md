...
>
> Load `.brs2spec/module-index.md` instead — it contains the Skill Index, trigger-to-skill lookup, loading rules, and stop rules in a compact startup form.
...
| qa-analyst | `qa.create_bdd_scenarios` | `skills/4-engineering-readiness/quality-gates/create-bdd-scenarios.md` | Create BDD scenarios with SCN-NNN IDs |
| qa-analyst | `qa.create_test_strategy` | `skills/4-engineering-readiness/quality-gates/create-test-strategy.md` | Create test-strategy.md — test approach, coverage, and risk |
| qa-analyst | `qa.generate_test_stubs_from_bdd` | `skills/8-copilot-implementation/03-generate-test-stubs-from-bdd.md` | Generate runnable failing test stubs from BDD scenarios |
...
1. Identify the current phase using `state/workflow-state.json` or the stage lookup table in Section 6.
...
6. Update `state/workflow-state.json` after the artifact is produced.
...
|---|---|---|---|---|
| 1 | Routing | `state/routing-decision.md` | Delivery mode, execution mode, and rationale stated | (routing prompt — no dedicated skill) |
...
| 7 | Open decisions | `state/open-decisions.md` | All decisions captured with owners; blocking summary accurate | `orchestrator.maintain_open_decisions` |
...
| 10 | Open decisions update | `state/open-decisions.md` | Readiness blockers recorded with owners | `orchestrator.maintain_open_decisions` |
...
| 13 | Handoff | `specs/` or `standalone-delivery/` | Dependency graph + all story folders; tasks traceable to stories | `engineering_lead.create_openspec_handoff` or `engineering_lead.create_standalone_handoff` |
...
| `state/open-decisions.md` | `orchestrator.maintain_open_decisions` | orchestrator |
...
| `state/workflow-state.json` | `orchestrator.maintain_state` | orchestrator |
...
| `quality-gates/bdd/` | `qa.create_bdd_scenarios` | qa-analyst |
...
| `specs/` | `engineering_lead.create_openspec_handoff` | engineering-lead |
...
4. **Blocking open decisions exist** (`state/open-decisions.md` has `blocking_count > 0`). Report the blocking decisions; stop.

...
def list_workflow_types(args: object) -> None:
    """Print available workflow types from the index."""
    index_path = workspace.FRAMEWORK_ROOT / "workflow-types" / "index.yaml"
    if not index_path.exists():
        raise FileNotFoundError(f"Workflow types index not found: {index_path}")
    index = yaml.safe_load(index_path.read_text(encoding="utf-8"))
    result = []
    for wt in index.get("workflow_types", []):
        result.append({
            "id": wt["id"],
            "description": wt.get("description", ""),
            "suitable_for": wt.get("suitable_for", []),
        })
...
def run(args: object) -> None:
    ...
    if not workflow_type:
        raise ValueError(
            "No --workflow-type specified. "
            "Run `list-workflow-types` to see available options, then pass one via --workflow-type."
        )
